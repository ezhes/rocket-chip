...
class TLBPTWIO(implicit p: Parameters) extends CoreBundle()(p)
  ...
  val disableDCacheClockGate = Input(Bool())
  val disableDCache = Input(Bool())
  val suppressCorruptOnGrantData = Input(Bool())
}
...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val clock_en = state =/= s_ready || l2_refill_wire || arb.io.out.valid || io.dpath.sfence.valid || io.dpath.disableDCacheClockGate
  ...
  for (i <- 0 until io.requestor.size) {
    ...
    io.requestor(i).disableDCacheClockGate := io.dpath.disableDCacheClockGate
    io.requestor(i).disableDCache := io.dpath.disableDCache
    io.requestor(i).suppressCorruptOnGrantData := io.dpath.suppressCorruptOnGrantData
